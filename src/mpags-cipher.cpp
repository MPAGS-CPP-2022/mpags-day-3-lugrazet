...
int main(int argc, char* argv[])
{
    ...
    if (!cmdLineSettings.cipherKey.empty()) {
        ...
        for (const auto& elem : cmdLineSettings.cipherKey) {
            if (!std::isdigit(elem)) {
                std::cerr
                    << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
                    << "        the supplied key (" << cmdLineSettings.cipherKey
                    << ") could not be successfully converted" << std::endl;
                return 1;
            }
        }
        caesarKey = std::stoul(cmdLineSettings.cipherKey);
    }
